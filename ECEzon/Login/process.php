...
if (isset($_POST['login'])) {
    ...
    elseif ($login!='admin' && !filter_var($login,FILTER_VALIDATE_EMAIL)) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
          ...
          elseif ($pwdcheck == true) {
            ...
            if ($row['Type'] == 'Client') {
              ...
              header("Location: ../Pages/account_client.html?success=LoginClient");
              exit();
            } // fin else if connection client
            elseif ($row['Type'] == 'Vendeur') {
              // Log to client dashboard
              header("Location: ../Pages/account_vendor.html?success=LoginVendeur");
              exit();
            } // fin elseif connection vendeur
            elseif ($row['Type'] == 'Admin') {
              // Log to client dashboard
              header("Location: ../Pages/account_admin.html?success=LoginAdmin");
              exit();
            ...
            // si pas de résultat dans la base de données alors creer un compte
