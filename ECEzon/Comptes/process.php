...
if (isset($_POST['signup'])) {
    ...
    // invalid email address + exception admin
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)) {
      ...
      if ($resultCheck > 0) {
        ...
        if (!mysqli_stmt_prepare($stmt,$sql)) {
          ...
          mysqli_stmt_execute($stmt);
          header("Location: ../Pages/account_client.html?signup=finished");
          exit();
        }
      }
    }
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)) {
      ...
      if ($resultCheck > 0) {
        ...
        if (!mysqli_stmt_prepare($stmt,$sql)) {
          ...
          mysqli_stmt_execute($stmt);
          header("Location: ../Pages/account_vendor.html?signup=finished");
          exit();
        }
      }
    }
